...
function searchUser_bSession($db, $session){
	if(!empty($session) && !mysqli_error($db)){
		$sql = mysqli_query($db, "select id,email,username,password,firstName,lastName,profilePicture,admin,verified from user where session = '$session'");
		if(mysqli_num_rows($sql) > 0){
			$result = mysqli_fetch_array($sql);
...
function checkUserSession($db){
	global $inLogin, $isAdmin, $isVerified,  $profilePicture;
