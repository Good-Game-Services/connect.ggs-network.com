...
if(checkUserSession($db) !== True){
	...
	if(!empty($_POST["session"])){
		...
		$sql = mysqli_query($db, "select * from user where session = '$session'") or die(json_encode(array("success" => false, "message" => "Error sql query")));
		
		if(mysqli_num_rows($sql) > 0){
			$user = searchUser_bSession($db, $session);
