	...
	if(!empty($_POST["cr_password"]) && !empty($_POST["nw_password"])){
		...
		if(password_verify($current_password, $user["password"])){
			if(strlen($new_password) >= 6){
				...
				$pasword_hashed = password_hash($new_password, PASSWORD_BCRYPT);
				mysqli_query($db, "UPDATE user SET session = '$userSession', password = '$pasword_hashed' WHERE username = '{$user["username"]}'") or die(json_encode(array("success" => false, "message" => "Error update sql query")));
