...
?>
        ...
        <div class="wrapper wrapper-content animated fadeIn">
            <div class="row">
				<!-- SYSTEM -->
                <div id="system_control" class="col-lg-12">
                        ...
                        <div class="ibox-content">
							...
							<form id="support_ticket" method="POST" action="" class="form-horizontal">
								<div class="form-group"><label class="col-sm-1 control-label">ID</label>
									<div class="col-sm-10"><input type="text" name="id" value="" class="form-control"></div>
								</div>
								<div class="form-group"><label class="col-sm-1 control-label">Ban Reason</label>
                    ...
                    <div class="ibox float-e-margins">
						...
						?>
								...
								<div class="social-avatar">
									<img alt="image" class="img-circle" src="<?= $_user["profilePicture"]; ?>" width="50px" height="50px" />
								</div>
								<div class="social-body">
									<div>
										<strong>ID: </strong><?= $_user["id"];?>
									</div>
									<div>
										<strong>Name: </strong><?= $_user["firstName"] . " " . $_user["lastName"]; ?>
									</div>
									<div>
										<strong>Username: </strong><?= $_user["username"];?>
									</div>
									<div>
								...
								<div class="file-option">
									...
									<button id="change-pb-admin-id-<?= $_user["id"] ?>" class="btn btn-info btn-rounded btn-sm" onclick="pb_user(<?= $_user["id"] ?>)"><i class="fa fa-level-up"></i> Change PB</button>
										</div>
										</div>
								</div>
							<?php endwhile;
...
function restart_api_server(){
	var c = confirm("Are you sure? Press OK if you wanna restart api server!");
	if (c == true) {
		$.ajax({
			url: "ajax/request/admin/restart_api_server.php",
			type: "POST",
			beforeSend: function () {
...
//reset users pb
function pb_user(user_id){
	var url;
	var input = prompt("Set new pb url:", "https://i.imgur.com/eHeR4ej.jpeg");
	url = input;
	
	if (url != null && url != "") {
	  $.ajax({
			url: "ajax/request/admin/pb_user.php",
			type: "POST",
			data: {
				user_id: user_id,
				url: url
